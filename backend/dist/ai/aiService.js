...
class AIService {
    static geminiKey = process.env.GEMINI_API_KEY || '';
    /**
     * Parse a text command in English or Hindi into a structured Task configuration.
     */
    static async parseTaskRequest(text, homeId) {
        if (this.geminiKey) {
            try {
     ...
     */
    static async suggestMealsFromPantry(homeId) {
        const pantry = await client_1.default.pantryItem.findMany({
            where: { homeId, isDeleted: false },
        });
        const recipes = await client_1.default.recipe.findMany({
        ...
        for (const recipe of recipes) {
            ...
            if (Array.isArray(ingredients)) {
                ...
                for (const ing of ingredients) {
                    const pantryItem = pantry.find((p) => p.name.toLowerCase() === ing.name.toLowerCase() && p.quantity >= (ing.quantity || 0));
                    if (pantryItem) {
                        matchedCount++;
                    }
                }
     ...
     */
    static async suggestGroceries(homeId) {
        const lowStockItems = await client_1.default.pantryItem.findMany({
            where: {
                homeId,
                isDeleted: false,
                quantity: {
                    lt: client_1.default.pantryItem.fields.minStock,
                },
            },
        });
     ...
     */
    static async chatAboutHome(message, homeId) {
        // Gather system state for context
        const openTasks = await client_1.default.task.count({ where: { homeId, status: { in: ['PENDING', 'IN_PROGRESS'] }, isDeleted: false } });
        const attendanceToday = await client_1.default.attendance.findMany({
            where: { date: new Date(), isDeleted: false },
            include: { staff: true },
        });
        const lowStockCount = await client_1.default.pantryItem.count({
            where: {
                homeId,
                isDeleted: false,
                quantity: { lt: client_1.default.pantryItem.fields.minStock },
            },
        });
        const visitorsToday = await client_1.default.visitor.count({
            where: {
                homeId,
                isDeleted: false,
                createdAt: {
...
`;
     ...
     */
    static async summarizeReport(reportContent) {
    ...
    static async callGeminiForTask(text, homeId) {
        const staff = await client_1.default.staff.findMany({ where: { isDeleted: false } });
...
`;
        ...
        return {
            ...
            priority: parsed.priority || client_2.Priority.MEDIUM,
            category: parsed.category || client_2.TaskCategory.OTHER,
            assignedStaffId: parsed.assignedStaffId || undefined,
            dueDate: parsed.dueDate ? new Date(parsed.dueDate) : undefined,
        };
    }
    static async callGeminiGeneral(prompt) {
    ...
    static async ruleBasedTaskParser(text, homeId) {
        const lowercaseText = text.toLowerCase();
        let category = client_2.TaskCategory.OTHER;
        let priority = client_2.Priority.MEDIUM;
        let assignedStaffId = undefined;
        let title = 'Household Task';
        let description = text;
        // Detect Category
        if (lowercaseText.includes('clean') ||
            ...
            category = client_2.TaskCategory.CLEANING;
            title = 'Cleaning Request';
        }
        else if (lowercaseText.includes('cook') ||
            ...
            category = client_2.TaskCategory.COOKING;
            title = 'Cooking Request';
        }
        else if (lowercaseText.includes('drive') ||
            ...
            category = client_2.TaskCategory.MAINTENANCE;
        ...
        if (!assignedStaffId) {
            ...
            if (category === client_2.TaskCategory.CLEANING)
                matchedType = 'HOUSEKEEPER';
            else if (category === client_2.TaskCategory.COOKING)
                matchedType = 'CHEF';
            else if (category === client_2.TaskCategory.DRIVING)
                matchedType = 'DRIVER';
            else if (category === client_2.TaskCategory.SECURITY)
