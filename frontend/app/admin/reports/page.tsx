...
export default function AdminReportsPage() {
  ...
  const generateReport = async (type: string) => {
    setGenerating(true);
    try {
      await api.post('/reports', { type, title: `${type.replace(/_/g,' ')} - ${new Date().toLocaleDateString()}` });
      fetchReports();
    } catch (e) { console.error(e); }
  ...
  const deleteReport = async (id: string) => {
    ...
    try { await api.delete(`/reports/${id}`); fetchReports(); }
    catch (e) { console.error(e); }
  };

  return (
    <div>
      <PageHeader title="Reports" description="Generate and review household operational reports"
        action={
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">{new Date(r.createdAt).toLocaleString()} · Generated by {r.generatedBy?.name}</p>
                ...
                <button onClick={() => deleteReport(r.id)} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg transition">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
