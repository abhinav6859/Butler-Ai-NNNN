...
import { ClipboardList, Clock, Calendar, Play, CheckCircle, Sparkles } from 'lucide-react';
...
export default function CleaningTasksPage() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // Fetch cleaning tasks assigned to this staff member
      const res = await api.get(`/tasks?assignedStaffId=${user.staffId}&category=CLEANING`);
  ...
  const handleUpdateStatus = async (id: string, status: string) => {
    ...
    try {
      await api.put(`/tasks/${id}`, {
        status,
        remarks: status === 'COMPLETED' ? 'Cleaning completed by housekeeper' : 'Cleaning started by housekeeper',
      });
  ...
  return (
    <div>
      <PageHeader title="Cleaning & Housekeeping Tasks" description="View and update your assigned cleaning zones and tasks" />
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800/60 rounded-2xl p-8 text-center">
          <Sparkles size={40} className="mx-auto text-slate-300 dark:text-slate-750 mb-3" />
          <p className="text-slate-500 dark:text-slate-400 font-semibold">No cleaning tasks assigned. Spotless work!</p>
        </div>
      ) : (
              ...
              <div className="flex items-center space-x-3 md:self-center ml-12 md:ml-0">
                {task.status === 'PENDING' && (
                  <button
                    ...
                    className="flex items-center space-x-1.5 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-755 text-white text-xs font-semibold rounded-xl disabled:opacity-60 transition"
                  >
                    <Play size={14} />
                ...
                {task.status === 'IN_PROGRESS' && (
                  <button
                    ...
                    className="flex items-center space-x-1.5 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-750 text-white text-xs font-semibold rounded-xl disabled:opacity-60 transition"
                  >
                    <CheckCircle size={14} />
