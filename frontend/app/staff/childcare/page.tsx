...
import { ClipboardList, Calendar, Play, CheckCircle, Baby } from 'lucide-react';
...
export default function ChildcareTasksPage() {
  const { user } = useAuth();
  ...
  const fetchTasks = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // Fetch childcare/other tasks assigned to this staff member
      const res = await api.get(`/tasks?assignedStaffId=${user.staffId}&category=OTHER`);
  ...
  const handleUpdateStatus = async (id: string, status: string) => {
    ...
    try {
      await api.put(`/tasks/${id}`, {
        status,
        remarks: status === 'COMPLETED' ? 'Childcare duty completed' : 'Childcare duty started',
      });
  ...
  return (
    <div>
      <PageHeader title="Childcare Duties" description="View and update your assigned childcare duties and schedules" />
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800/60 rounded-2xl p-8 text-center">
          <Baby size={40} className="mx-auto text-slate-300 dark:text-slate-750 mb-3" />
          <p className="text-slate-500 dark:text-slate-400 font-semibold">No childcare duties assigned right now.</p>
        </div>
      ) : (
